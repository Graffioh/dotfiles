...
Before spawning investigators, inspect the available Codex tools, installed plugin tools, apps, and MCP resources exposed in the current session. Do not assume a connector exists merely because a source category is listed below.
...
- `task_name`: a short unique name for the evidence category
- `model`: your configured why-investigators model (default `gpt-5.6-luna`)
- prompt constraint: do not write files or mutate external systems. Investigators may use available read tools and connectors.
...
- `task_name`: `why_synthesizer`
- `model`: your configured why-synthesizer model (default `gpt-5.6-terra`)
- prompt constraint: read and report only; do not edit files or mutate external systems
