...
Use Codex's task tools to list and read recent tasks for the active workspace. If those tools are unavailable, use only the current conversation and task links or IDs the user supplied. Never scan unrelated workspaces or raw user history broadly.
...
3. Fan out across relevant task history. Use the app's list/read task tools to select tasks by workspace, recency, and topic. Give each subagent a bounded list of task IDs; skip the current task and obvious eval/test noise. Each returns the same schema, one block per task: topic, user goal, decisions, open threads, corrections, and artifacts such as PRs, tickets, and branches. For one or two tasks, skip fan-out and read directly. Keep raw task contents out of the main thread.
