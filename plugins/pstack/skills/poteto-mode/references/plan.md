...
Create a plan with one item per step below.
...
State your read of scope and constraints in one paragraph. Use `structured question tool` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Use a short lowercase `task_name` and tell each implementer to read `$poteto-mode` before working.
- Pass `model:` explicitly per the configured roles (defaults `gpt-5.6-luna` for code, `gpt-5.6-terra` for judgment).
...
If a phase creates or edits a skill, the phase instructs the implementer to use Codex.s **skill-creator** skill.
...
- Browser / Electron / Web UIs: the in-app browser or Playwright.
- CLIs and TUIs: terminal execution.
...
- a focused code-cleanup pass over each diff before commit. the **unslop** skill over any prose surface.
