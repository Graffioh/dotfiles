...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a worker-agent URL handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail from a Codex task ID or link, a subagent result, or a pushed branch. Use task-reading tools when available. Read the overview and last messages first, then scan back for decision points. Parse a long task in a subagent and keep only the reduced timeline in the main thread.
