...
6. **Verify the chain from task evidence, not self-report.** Use each candidate's subagent result and the current task record when thread tools expose it. Never scan unrelated tasks. Check which files each candidate actually opened or changed. Grade chain-following from observable actions and the output, not the candidate's claims.
