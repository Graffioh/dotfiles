...
This skill orchestrates three others: an inline mining pass (see step 1), Codex.s built-in **skill-creator** (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.codex/skills/**/*-mode/SKILL.md` and `~/.codex/skills/**/*-mode/SKILL.md` matching the user's handle. If one exists, confirm intent with a structured question tool when available, unless they already said "update my skill" or similar:
...
Use Codex's task tools to list and read recent tasks for the active workspace. Never scan unrelated workspaces. If task tools are unavailable, ask whether to proceed from the current conversation only.
...
Mining misses intent that hasn't come up yet. Use a structured question tool when available. Otherwise ask one concise question at a time.
...
Use Codex.s built-in **skill-creator** skill to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, default to `.codex/skills/<handle>-mode/` for a project skill or `~/.codex/skills/<handle>-mode/` for a personal skill.
...
- Put `policy.allow_implicit_invocation: false` in `agents/openai.yaml` by default. Mode skills are heavy and opinionated, so they should activate only when explicitly invoked unless the user asks otherwise.
...
- Codex.s built-in **skill-creator** skill: skill authoring process and writing guidelines.
