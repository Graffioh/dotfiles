...
Spawn all explorers concurrently:

- `task_name`: a short unique name such as `how_state` or `how_request_path`
...
- `task_name`: `how_explainer`
- `model`: your configured how-explainer model (default `gpt-5.6-terra`)
- prompt constraint: read and report only; do not edit files
...
After the explanation is complete, spawn one architectural critic per model in your configured how-critics list (defaults `gpt-5.6-terra`, `gpt-5.6-sol`, `gpt-5.6-luna`, `gpt-5.5`), all in a single message.

For each critic:
- `task_name`: a unique name such as `how_critic_a`
