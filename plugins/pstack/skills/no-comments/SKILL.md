---
name: no-comments
description: "Run a fresh, read-only comment review, fix accepted findings, and offer encodings for claimed constraints. Use for $no-comments or pre-review comment cleanup."
---

# No comments

Spawn a fresh reviewer using the bundled Comment Sicko prompt. Act on accepted findings.
...
1. Read `references/comment-sicko.md` in full. Spawn a standard Codex subagent with task name `comment_sicko`, pass the scope, and instruct it to follow that file verbatim. The reviewer reports findings and must not edit application code.
