...
The human enters setup by pointing Cursor at the pack's `FOR_AGENTS.md`. The bootstrap flow copies the whole pack into the target repository, then reads this file directly at `.codex/automations/benny/skills/setup-benny/SKILL.md`.
...
Add pstack to the target repository's `.codex/config.toml`. If the file or `.cursor` directory does not exist, create it.
...
Tell the user that `.codex/config.toml`, `.codex/automations/benny/`, and any referenced secret-free configuration must be committed before either automation is enabled. Do not commit them unless the user asks.
...
Before testing, confirm that the target repository's `.codex/config.toml`, `.codex/automations/benny/`, and every referenced secret-free configuration file are committed on the branch used by the automation checkout. Confirm that both live prompts point at their exact committed operational files. If any check fails, stop. Tell the user that the automation cannot be enabled yet.
