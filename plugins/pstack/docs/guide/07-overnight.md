...
the recurring monitoring or wait mechanism until done. if you're truly stuck after a few hours, stop and write up why.
```
...
- the recurring monitoring or wait mechanism is Cursor's built-in wake mechanism, not a pstack skill. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses it to re-check the finish condition on events or a heartbeat.
...
**Pitfall:** a duration is not a finish condition. "work on this for 4 hours" gives the agent nothing to check, and you'll wake up to four hours of motion instead of a result. Give the recurring monitoring or wait mechanism a predicate that can pass or fail.
