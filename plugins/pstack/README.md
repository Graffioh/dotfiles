...
> Codex adaptation of [Lauren Tan's pstack](https://github.com/cursor/plugins/tree/main/pstack), preserved under the MIT license. The engineering principles and playbooks remain upstream-derived; Cursor-specific manifests, models, subagents, transcript access, and invocation syntax were translated for Codex.
...
Install from this repository's Codex marketplace with `codex plugin add pstack@graffioh`.
...
Codex does not load Cursor's custom `subagent_type` declarations. `$poteto-mode` tells ordinary Codex subagents to read the mode skill before working. `$no-comments` loads the bundled [Comment Sicko prompt](./skills/no-comments/references/comment-sicko.md) into a fresh reviewer.
...
The adaptation uses Codex's skill creator, terminal, in-app browser, Playwright, plans, task tools, and collaboration subagents. Cursor-only `subagent_type`, `Task`, `/loop`, model, and transcript conventions were translated.

## why are there no planning skills?

Codex plan mode works well with pstack. [`$poteto-mode`](./skills/poteto-mode/SKILL.md) also carries its playbook steps in the task plan.
...
models are configurable too. type [`$setup-pstack`](./skills/setup-pstack/SKILL.md). it writes `~/.codex/pstack-models.md`, which orchestration skills read explicitly.
...
The Benny pack remains upstream reference material. It is not registered or activated because it depends on Slack and tracker connectors plus explicit authorization for external writes.
